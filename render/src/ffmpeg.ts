...
export interface MusicMixOptions {
  /** Music bed level relative to the loudnorm'd voice, in dB (negative = quieter). */
  musicGainDb: number;
  /** How hard the music ducks while the speaker talks, in dB (larger = more dip). */
  duckAmountDb: number;
  fadeInMs: number;
  fadeOutMs: number;
  /** Exact video length so the music is trimmed/faded to match. */
...
/**
 * Mix a background music track UNDER the finished video's voice, ducked via sidechain compression
 * so the music dips whenever the speaker talks. The video stream is copied untouched (`-c:v copy`)
 * and the voice keeps its existing loudnorm — we only add a low, ducked, faded music bed. The track
 * is stream-looped to cover the whole video, then trimmed to its exact length.
 */
export async function mixMusicUnderVoice(
  videoPath: string,
  trackPath: string,
  outPath: string,
  opts: MusicMixOptions,
): Promise<void> {
  const durS = Math.max(0.1, opts.videoDurationMs / 1000);
  const fadeIn = clampNum(opts.fadeInMs / 1000, 0, durS / 2);
  const fadeOut = clampNum(opts.fadeOutMs / 1000, 0, durS / 2);
  const fadeOutStart = Math.max(0, durS - fadeOut);
  // The library is pre-normalized to ~-20 LUFS and the voice to -14 LUFS, so the track already sits
  // under the voice at unity. musicGainDb is only a small trim around 0 — large negative values would
  // bury the bed. Keep it a trim, never an attenuation to silence.
  const gainDb = clampNum(opts.musicGainDb, -12, 3);
  // Map the requested duck depth onto a sidechain ratio (more dB -> higher ratio -> deeper duck),
  // capped so speech dips the bed without crushing it to nothing.
  const ratio = clampNum(1 + Math.abs(opts.duckAmountDb), 2, 8);

  const filter = [
    `[1:a]atrim=0:${durS.toFixed(3)},asetpts=N/SR/TB,` +
      `volume=${gainDb.toFixed(2)}dB,` +
      `afade=t=in:st=0:d=${fadeIn.toFixed(3)},` +
      `afade=t=out:st=${fadeOutStart.toFixed(3)}:d=${fadeOut.toFixed(3)}[music]`,
    `[0:a]asplit=2[voice][key]`,
    `[music][key]sidechaincompress=threshold=0.05:ratio=${ratio.toFixed(2)}:attack=20:release=300[ducked]`,
...
/** True when the file has at least one audio stream (music mix needs a voice track to duck under). */
