// Fast single-scene probe: global plan -> author -> render -> vision-QA for ONE scene, reusing an
// EXISTING clean base and transparent caption overlay. Skips transcribe/cut/caption-render entirely.
//
...
//
// Usage:
//   npx tsx scripts/probe-scene.ts <base.mp4> <captions.mov> <props.json> <fixture.json> [sceneIndex]
...
const [basePathArg, captionsPathArg, propsPathArg, fixturePathArg, sceneIndexArg] = process.argv.slice(2);

if (!basePathArg || !captionsPathArg || !propsPathArg || !fixturePathArg) {
  console.error("usage: npx tsx scripts/probe-scene.ts <base.mp4> <captions.mov> <props.json> <fixture.json> [sceneIndex]");
  process.exit(1);
}
...
async function main() {
  ...
  const captionOverlayPath = resolve(captionsPathArg);
  ...
  const editPlan = await planEdit({ brief, words: props.words, durationMs: props.durationMs, assetHints });
  const specs = editPlan.scenes;

  console.log(`\n=== scene probe ===`);
  console.log(`base    : ${basePath}`);
  console.log(`planned : ${specs.length} scenes, ${assetHints.length} assets`);
  if (!specs.length) throw new Error("planEdit intentionally produced no enhanced scenes");
  ...
  const scene = await produceScene({
    spec, brief, creativeDirection: editPlan.creativeDirection, assetHints, assetsDir, premiumDir,
    basePath, captionOverlayPath, fps: 30,
